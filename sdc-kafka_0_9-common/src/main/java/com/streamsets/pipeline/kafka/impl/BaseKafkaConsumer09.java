...
public abstract class BaseKafkaConsumer09 implements SdcKafkaConsumer {
  ...
  private static class DataCollectorConsumerListener implements ConsumerRebalanceListener {
    ...
    public DataCollectorConsumerListener(AtomicBoolean rebalanceInProgress, ArrayBlockingQueue<ConsumerRecord<String, byte[]>> recordQueue) {
  ...
  public BaseKafkaConsumer09(String topic, Source.Context context) {
    ...
    context.createGauge("Internal state", new Gauge<Map<String, Object>>() {
      @Override
  ...
  protected void createConsumer() {
    ...
    kafkaConsumer.subscribe(Collections.singletonList(topic), new DataCollectorConsumerListener(rebalanceInProgress, recordQueue));
  }
