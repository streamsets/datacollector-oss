...
public class TestSparkExecutor extends BaseSparkExecutorTest {
  ...
  private void verifyAppArgs() {
    verify(launcher, times(1))
        .addAppArgs(conf.appArgs.toArray(new String[conf.appArgs.size()]));
  ...
  public void testEL() throws Exception {
    ...
    conf.appArgs = ImmutableList.of("${record:value(\"/king\")}", "${record:value(\"/precious\")}");
  ...
  public void testEmptyEL() throws Exception {
    ...
    conf.appArgs = ImmutableList.of("${record:value(\"/king\")}", "${record:value(\"/ringbearer\")}");
