...
public class TestTextCharDataParser {
  ...
  public void testParse() throws Exception {
    ...
    DataParser parser = new TextCharDataParser(getContext(), "id", false, reader, 0, 1000, "text", "truncated");
  ...
  public void testParseWithOffset() throws Exception {
    OverrunReader reader = new OverrunReader(new StringReader("Hello\nBye"), 1000, true, false);
    DataParser parser = new TextCharDataParser(getContext(), "id", false, reader, 6, 1000, "text", "truncated");
    Assert.assertEquals(6, Long.parseLong(parser.getOffset()));
    Record record = parser.parse();
    Assert.assertNotNull(record);
    Assert.assertEquals("id::6", record.getHeader().getSourceId());
  ...
  public void testTruncate() throws Exception {
    OverrunReader reader = new OverrunReader(new StringReader("Hello\nBye"), 1000, true, false);
    DataParser parser = new TextCharDataParser(getContext(), "id", false, reader, 0, 3, "text", "truncated");
    Assert.assertEquals(0, Long.parseLong(parser.getOffset()));
    Record record = parser.parse();
    Assert.assertNotNull(record);
    Assert.assertEquals("id::0", record.getHeader().getSourceId());
  ...
  public void testOverrun() throws Exception {
    ...
    int lines = 0;
    try (DataParser parser = new TextCharDataParser(getContext(), "id", false, reader, 0, 3, "text", "truncated")) {
