...
public class TestDoubleMaxAggregator {

  @Test
  public void testAggregator() {
    Aggregators aggregators = new Aggregators(2, WindowType.ROLLING);
    DoubleMaxAggregator aggregator = aggregators.createSimple("a", DoubleMaxAggregator.class);
    aggregators.start(1);

    Aggregators aggregatorsA = new Aggregators(2, WindowType.ROLLING);
    DoubleMaxAggregator aggregatorA = aggregatorsA.createSimple("a", DoubleMaxAggregator.class);
    ...
    Assert.assertEquals((Double) 2d, aggregator.get());

    Assert.assertEquals("a", aggregator.getAggregatable().getName());
    Assert.assertEquals(DoubleMaxAggregator.DoubleMaxAggregatable.class.getSimpleName(), aggregator.getAggregatable().getType());
    Assert.assertEquals(2d, ((DoubleMaxAggregator.DoubleMaxAggregatable) aggregator.getAggregatable()).getMax(), 0.0001);

    aggregatorA.process(1d);

    aggregatorA.aggregate(aggregator.getAggregatable());
    Assert.assertEquals(2d, aggregatorA.get(), 0.000001);
