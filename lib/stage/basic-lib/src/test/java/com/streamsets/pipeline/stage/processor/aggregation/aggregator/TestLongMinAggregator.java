...
public class TestLongMinAggregator {
  ...
  public void testAggregator() {
    ...
    Assert.assertEquals(LongMinAggregator.LongMinAggregatable.class.getSimpleName(), aggregator.getAggregatable().getType());
    Assert.assertEquals(1L, ((LongMinAggregator.LongMinAggregatable) aggregator.getAggregatable()).getMin());

    aggregatorA.process(3L);

    aggregatorA.aggregate(aggregator.getAggregatable());
    Assert.assertEquals((Long) 1L, aggregatorA.get());
