...
public class TestJdbcOracleCDCRecordReader {
  ...
  @Test
  public void testGetOperationFromRecordOracleOperationCode() throws Exception {
    List< OnRecordErrorException > errors = new ArrayList<>();
    JdbcRecordReader msReader = new JdbcOracleCDCRecordReader();

    Record r1 = RecordCreator.create();
    Map<String, Field> insert = new HashMap<>();
    r1.set(Field.create(insert));
    // Only oracle.cdc.operation code is set in record header.
    // Oracle code should be converted to SDC standard code
    r1.getHeader().setAttribute(
        OracleCDCOperationCode.OPERATION,
        String.valueOf(OracleCDCOperationCode.INSERT_CODE)
    );
    assertEquals(
        OperationType.INSERT_CODE,
        msReader.getOperationFromRecord(r1, JDBCOperationType.DELETE, UnsupportedOperationAction.DISCARD, errors)
    );

    Record r2 = RecordCreator.create();
    r2.set(Field.create(insert));
    r2.getHeader().setAttribute(
        OracleCDCOperationCode.OPERATION,
        String.valueOf(OracleCDCOperationCode.UPDATE_CODE)
    );
    assertEquals(
        OperationType.UPDATE_CODE,
        msReader.getOperationFromRecord(r2, JDBCOperationType.DELETE, UnsupportedOperationAction.DISCARD, errors)
    );

    Record r3 = RecordCreator.create();
    r3.set(Field.create(insert));
    r3.getHeader().setAttribute(
        OracleCDCOperationCode.OPERATION,
        String.valueOf(OracleCDCOperationCode.DELETE_CODE)
    );
    assertEquals(
        OperationType.DELETE_CODE,
        msReader.getOperationFromRecord(r3, JDBCOperationType.INSERT, UnsupportedOperationAction.DISCARD, errors)
    );

    Record r4 = RecordCreator.create();
    r4.set(Field.create(insert));
    r4.getHeader().setAttribute(
        OracleCDCOperationCode.OPERATION,
        String.valueOf(OracleCDCOperationCode.SELECT_FOR_UPDATE_CODE)
    );
    assertEquals(
        OperationType.UPDATE_CODE,
        msReader.getOperationFromRecord(r4, JDBCOperationType.INSERT, UnsupportedOperationAction.DISCARD, errors)
    );
  ...
  public void testGetOperationFromRecordUnsupportedOperation() throws Exception {
    ...
    insert.put("P_ID", Field.create(300));
    r1.set(Field.create(insert));
    r1.getHeader().setAttribute(OperationType.SDC_OPERATION_TYPE, "wrong code");

    List< OnRecordErrorException > errors = new ArrayList<>();

    assertEquals(
        OperationType.INSERT_CODE,
        msReader.getOperationFromRecord(r1, JDBCOperationType.INSERT, UnsupportedOperationAction.USE_DEFAULT, errors)
    );

    Record r2 = RecordCreator.create();
    r2.set(Field.create(insert));
    r2.getHeader().setAttribute(OperationType.SDC_OPERATION_TYPE, String.valueOf(OperationType.UNSUPPORTED_CODE));
    assertEquals(
        OperationType.DELETE_CODE,
        msReader.getOperationFromRecord(r2, JDBCOperationType.DELETE, UnsupportedOperationAction.USE_DEFAULT, errors)
    );

    assertEquals(
        -1,
        msReader.getOperationFromRecord(r2, JDBCOperationType.UPDATE, UnsupportedOperationAction.DISCARD, errors)
    );
    assertEquals(0, errors.size());
  }
