...
public class TestHttpToKafka {

  private static List<KafkaStream<byte[], byte[]>> kafkaStreams1;

  private static final int PARTITIONS = 3;
  private static final int REPLICATION_FACTOR = 2;
  private static final String TOPIC1 = "TestHttpToKafka1";
  ...
  public void testWrite() throws Exception {
    ...
    // we need to do this because it is done in the DTarget
    configs.setMaxHttpRequestSizeKB(900);

    final HttpToKafkaSource source = new HttpToKafkaSource(configs, kafkaConfigBean, 900);

    // create source runner
    final SourceRunner sourceRunner = new SourceRunner.Builder(HttpToKafkaDSource.class, source)
        .addOutputLane("lane")
        .build();
    ...
    try {
      ...
      //ping
      URL url = new URL("http://localhost:" + randomPort + "/");
      ...
      // send request
      url = new URL("http://localhost:" + randomPort + "/");
