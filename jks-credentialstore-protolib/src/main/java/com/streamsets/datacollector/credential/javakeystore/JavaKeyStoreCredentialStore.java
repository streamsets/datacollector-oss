...
public class JavaKeyStoreCredentialStore implements CredentialStore {
  ...
  public List<ConfigIssue> init(Context context) {
    ...
    if (issues.isEmpty()) {
      ...
      try {
        if (getKeyStore() == null) {
          setKeyStore(createKeyStore());
        }
        if (getEntry(SSH_PRIVATE_KEY_SECRET) == null) {
  ...
  class JksCredentialValue implements CredentialValue {
    ...
    public String get() throws StageException {
      ...
      if (keyStore == null) {
        ...
        try {
          KeyStore.SecretKeyEntry entry = getEntry(name);
          if (entry != null) {
  ...
  @VisibleForTesting
  protected SshUtils.SshKeyInfoBean createSshKeyInfo() {
