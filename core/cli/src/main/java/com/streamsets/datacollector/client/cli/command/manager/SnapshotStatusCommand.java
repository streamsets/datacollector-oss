...
@Command(name = "snapshot-status", description = "Get Snapshot Status")
public class SnapshotStatusCommand extends BaseCommand {
  ...
  public void run() {
    ...
    try {
      ...
      System.out.println(mapper.writeValueAsString(managerApi.getSnapshotStatus(pipelineId, snapshotName,
        pipelineRev)));
    } catch (Exception ex) {
      if(printStackTrace) {
        ex.printStackTrace();
