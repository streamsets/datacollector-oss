...
@Command(name = "get-rules", description = "Get Pipeline Rules")
public class GetPipelineRulesCommand extends BaseCommand {
  @Option(
    name = {"-n", "--name"},
  ...
  public void run() {
    ...
    try {
      ...
      System.out.println(mapper.writeValueAsString(storeApi.getPipelineRules(pipelineId, pipelineRev)));
    } catch (Exception ex) {
      if(printStackTrace) {
        ex.printStackTrace();
      } else {
