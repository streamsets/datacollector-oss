...
@Command(name = "stop", description = "Stop Preview")
public class StopPreviewCommand extends BaseCommand {
  @Option(
    name = {"-n", "--name"},
  ...
  public void run() {
    try {
      ...
      System.out.println(mapper.writeValueAsString(previewApi.stopPreview(pipelineName, previewerId)));
    } catch (Exception ex) {
      if(printStackTrace) {
        ex.printStackTrace();
      } else {
