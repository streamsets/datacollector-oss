...
public class ClusterRunner implements Runner {
  ...
  @Inject
  public ClusterRunner(String name, String rev, String user, RuntimeInfo runtimeInfo,
    ...
    this(name, rev, user, runtimeInfo, configuration, pipelineStore, pipelineStateStore, stageLibrary, executorService,
      null);
  }
  ...
  private synchronized void validateAndSetStateTransition(PipelineStatus toStatus, String message, Map<String, Object> attributes)
    ...
    final PipelineStatus status = getStatus();
    if (status == toStatus) {
  ...
  static class ManagerRunnable implements Runnable {
    ...
    private void checkStatus() throws PipelineStoreException, PipelineRunnerException {
      if (clusterRunner.getStatus().isActive()) {
  ...
  private void scheduleRunnable() throws PipelineStoreException {
    ...
    managerRunnableFuture =
      executor.scheduleAtFixedRate(new ManagerRunnable(this, getPipelineConf()), 0, 30, TimeUnit.SECONDS);
  }
