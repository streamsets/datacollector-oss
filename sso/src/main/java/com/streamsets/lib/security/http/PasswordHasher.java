...
public class PasswordHasher {
  ...
  static final SecretKeyFactory SECRET_KEY_FACTORY;
  ...
  static {
    try {
      SECRET_KEY_FACTORY = SecretKeyFactory.getInstance(HASH_ALGORITHM);
  ...
  protected String[] createRandomValueAndHash() {
  ...
  public String[] getRandomValueAndHash() {
    return createRandomValueAndHash();
  }

  public String getPasswordHash(String user, String password) {
    int iterations = getIterations();
    byte[] salt = getSalt();
    return computeHash(V2, iterations, salt, getValueToHash(V2, user, password));
  }
  ...
  protected String getValueToHash(String hashVersion, String user, String password) {
    switch (hashVersion) {
      ...
      case V2:
        return user + "\n" + password;
      default:
        throw new IllegalArgumentException(Utils.format("Invalid hash version '{}'", hashVersion));
    }
  }
      ...
      try {
        ...
        if (parts.length > 0) {
          switch (version) {
            ...
            break;
            default:
              throw new IllegalArgumentException(Utils.format("Invalid hash version '{}'", version));
          }
        }
      } catch (Exception ex) {
        throw new RuntimeException(ex);
