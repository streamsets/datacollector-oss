...
public class RedisSubscriptionSource extends BaseRedisSource {
    ...
    protected List<ConfigIssue> init() {
        ...
        if (createRedisClient(issues)) {
            ...
            if (null != subscriptionPatterns
                    && !subscriptionPatterns.isEmpty()) {
                if (redisListener.isSubscribed()) {
    ...
    public String produce(String lastSourceOffset, int maxBatchSize, BatchMaker batchMaker) throws StageException {
        ...
        while (recordCounter < maxBatchSize && (startTime + conf.maxWaitTime) > System.currentTimeMillis()) {
    ...
    private boolean checkSubscribe(final List<ConfigIssue> issues) {
        ...
        try {
            ...
            int i = 0;
            while (!isOk.get()
                    && i < conf.advancedConfig.connectionTimeout) {
                if (redisListener.isSubscribed()) {
                    isOk.set(true);
                    break;
                }
            ...
            Runnable psubscribeRunnable = new Runnable() {
                ...
                public void run() {
                    while (!Thread.currentThread().isInterrupted()) {
                        redisClient.psubscribe(redisListener, subscriptionPatterns.toArray(new String[subscriptionPatterns.size()]));
                    }
                }
            };
