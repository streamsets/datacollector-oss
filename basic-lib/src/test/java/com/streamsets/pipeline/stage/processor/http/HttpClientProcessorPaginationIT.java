...
public class HttpClientProcessorPaginationIT extends JerseyTest {
  private static final Logger LOG = LoggerFactory.getLogger(com.streamsets.pipeline.stage.processor.http.HttpClientProcessorPaginationIT.class);
  ...
  protected Application configure() {
    ...
    return new ResourceConfig(com.streamsets.pipeline.stage.processor.http.HttpClientProcessorPaginationIT.PagingResource.class);
  }

  @Override
  protected DeploymentContext configureDeployment() {
    return ServletDeploymentContext.forServlet(new ServletContainer(new ResourceConfig(com.streamsets.pipeline.stage.processor.http.HttpClientProcessorPaginationIT.PagingResource.class))).build();
  }
  ...
  public void testPaging() throws Exception {
    HttpProcessorConfig conf = getHttpClientConfigBean(pageNum, limit, mode, keepAllFields);

    HttpProcessor processor = new HttpProcessor(conf);
    ...
    Record inputRecord = runner.getContext().createRecord("");
    inputRecord.set(Field.create(new HashMap()));

    try {

      StageRunner.Output output = runner.runProcess(Arrays.asList(inputRecord));
      Map<String, List<Record>> recordMap = output.getRecords();
      String pathField = "/out";
      if(keepAllFields)
        pathField += "[0]/results";
      List<Field> parsedRecords = (List<Field>) recordMap.get("lane").get(0).get(pathField).getValue();

      int start;
      if (mode != BY_OFFSET) {
        start = (pageNum - 1) * limit;
        int size = parsedRecords.size();
        assertEquals(10 - start, size);
      } else {
        start = pageNum;
      ...
      int recordNum = start;
      for (Field field : parsedRecords) {
        String rowFieldPath = keepAllFields ? conf.pagination.resultFieldPath + "/row" : "/row";
        assertEquals(++recordNum, field.getValueAsMap().get("row").getValueAsInteger());
        if (keepAllFields) {
          assertTrue(recordMap.get("lane").get(0).has("/out[0]/metadata"));
        }
      }
    } finally {
      runner.runDestroy();
  ...
  private HttpProcessorConfig getHttpClientConfigBean(
      int start,
      int limit,
      PaginationMode mode,
    ...
    conf.outputField = "/out";
    conf.multipleValuesBehavior = MultipleValuesBehavior.ALL_AS_LIST;
    conf.headerOutputLocation = HeaderOutputLocation.FIELD;
    conf.headerOutputField = "/httpResHeaders";
