...
public abstract class AbstractCoapServerProtoSource<R extends CoapReceiver, C extends ProtoSource.Context>
    extends BaseStage<C> {
  private final CoapServerConfigs coAPServerConfigs;

  private R receiver;
  private CoapReceiverServer server;

  private BlockingQueue<Exception> errorQueue;
  private List<Exception> errorList;

  AbstractCoapServerProtoSource(CoapServerConfigs coAPServerConfigs, R receiver) {
  ...
  CoapServerConfigs getCoAPServerConfigs() {
    return coAPServerConfigs;
  }
  ...
  protected List<ConfigIssue> init() {
    ...
    if (issues.isEmpty()) {
      ...
      server = new CoapReceiverServer(coAPServerConfigs, receiver, getErrorQueue());
  ...
  @VisibleForTesting
  private BlockingQueue<Exception> getErrorQueue() {
    return errorQueue;
  }

  void dispatchHttpReceiverErrors(long intervalMillis) {
    if (intervalMillis > 0) {
      try {
    ...
    // report errors  reported by the CoapReceiverServer
    errorList.clear();
